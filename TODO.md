# List of things I would like to do in this project.

## 2026-03-27 | March 27, 2026

Meme Generator for Matt.

1. ~~Figure out how to center sources.~~
1. ~~Figure out how to listen to chat for matt's first time messages.~~
1. ~~Populate with stupid matt messages and images.~~

## 2026-03-25 | March 25, 2026

Meme Generator for Matt.

1. ~~Set up Python environment.~~
1. ~~Figure out how tf to connect to OBS websocket.~~
1. ~~Setup twitch bot~~
1. ~~Figure out how to listen to Twitch Chat~~
1. ~~Figure out how to get sources from OBS.~~
1. ~~Figure out how to edit sources from OBS.~~
1. Figure out how to center sources.
1. Figure out how to listen to chat for matt's first time messages.
1. Populate with stupid matt messages and images.